    ...
    try:
        ...
        for key in required_keys:
            if key not in data or data[key] is None:
                logger.error(f"Predict missing/null field '{key}'")
                return jsonify({'success': False, 'error': f'Missing required field: {key}'}), 400
            try: float(data[key])
            except (ValueError, TypeError):
                 logger.error(f"Predict invalid value '{data[key]}' for '{key}'.")
        ...
        if predicted_aqi is not None:
            ...
            return jsonify({
             ...
             logger.error(f"Predict function returned None for data: {data}")
             return jsonify({'success': False, 'error': 'Prediction failed. Check logs.'}), 500
...
# --- NEW: Top Cities AQI Endpoint (Missing from previous attempts) ---
...
def top_cities_aqi():
    ...
    world_cities = [
        "Beijing", "New York", "London", "Tokyo", "Paris",
        "Los Angeles", "Mexico City", "Sao Paulo", "Cairo", "Moscow",
        "Jakarta", "Seoul", "Sydney", "Berlin", "Rome"
    ]
    ...
    for city_name in indian_cities:
        ...
        if 'error' in coords:
            logger.warning(f"Skipping Indian city {city_name} (geocoding error): {coords['error']}")
            continue
        aqi_data = fetch_aqi(coords['lat'], coords['lon'], coords['name'])
        if 'error' not in aqi_data:
            top_cities_data['india'].append({
                'city': aqi_data.get('city'),
                'aqi': aqi_data.get('aqi'),
                'category': get_aqi_category(aqi_data.get('aqi', -1))
            })
        else:
            logger.warning(f"Skipping Indian city {city_name} (AQI fetch error): {aqi_data.get('error')}")

    # Fetch data for World cities
    for city_name in world_cities:
        coords = get_coords_from_city(city_name)
        if 'error' in coords:
            logger.warning(f"Skipping World city {city_name} (geocoding error): {coords['error']}")
            continue
        aqi_data = fetch_aqi(coords['lat'], coords['lon'], coords['name'])
        if 'error' not in aqi_data:
             top_cities_data['world'].append({
                'city': aqi_data.get('city'),
                'aqi': aqi_data.get('aqi'),
                'category': get_aqi_category(aqi_data.get('aqi', -1))
            })
        else:
            logger.warning(f"Skipping World city {city_name} (AQI fetch error): {aqi_data.get('error')}")
...
def get_city_data(city_from_url):
    ...
    logger.info(f"[get_city_data] Calling fetch_aqi with lat={coords['lat']}, lon={coords['lon']}, name='{official_city_name}'")
    aqi_data = fetch_aqi(coords['lat'], coords['lon'], official_city_name)
    logger.info(f"[get_city_data] Calling fetch_weather with lat={coords['lat']}, lon={coords['lon']}, name='{official_city_name}'")
    weather_data = fetch_weather(coords['lat'], coords['lon'], official_city_name)
    logger.info(f"[get_city_data] AQI data fetched: {aqi_data}")
    logger.info(f"[get_city_data] Weather data fetched: {weather_data}")
    ...
    logger.info(f"[get_city_data] Final combined data for '{official_city_name}': {aqi_data}")
...
def autocomplete_city():
    ...
    try:
        ...
        logging.warning(f"Autocomplete request timed out for query: {query}")
        return jsonify([])
    except requests.exceptions.RequestException as e:
        logging.error(f"Autocomplete API request error for query '{query}': {e}")
        return jsonify([])
    except Exception as e:
        logging.exception(f"Unexpected error in autocomplete_city for query '{query}': {e}")
        return jsonify({"error": "Autocomplete service error"}), 500

    return jsonify(suggestions)
# --- END NEW AUTOCOMPLETE ENDPOINT ---
...
def get_city_from_coords():
    ...
    try:
        ...
        if data and isinstance(data, list) and len(data) > 0:
            ...
            if not city_name:
                 logger.warning(f"Reverse geocoding no name: {data}")
            ...
            logger.warning(f"Reverse geocoding no data for ({lat},{lon}).")
        ...
        return jsonify({"error": f"Could not connect to location service."}), 503
    except Exception as e:
        logger.exception(f"Unexpected error in get_city_from_coords ({lat},{lon}): {e}")
